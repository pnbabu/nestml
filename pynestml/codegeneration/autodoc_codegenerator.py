...
class AutoDocCodeGenerator(CodeGenerator):
    ...
    def generate_code(self, neurons: List[ASTNeuron], synapses: List[ASTSynapse] = None) -> None:
        ...
        """
        dirs = ['neurons', 'synapses']
        for _dir in dirs:
            path = os.path.join(FrontendConfiguration.get_target_path(), _dir)
    ...
    def generate_neuron_code(self, neuron: ASTNeuron):
        ...
        with open(str(os.path.join(FrontendConfiguration.get_target_path(), 'neurons', neuron.get_name())) + '.rst',
                  'w+') as f:
    ...
    def generate_synapse_code(self, synapse: ASTSynapse):
        ...
        with open(str(os.path.join(FrontendConfiguration.get_target_path(), 'synapses', synapse.get_name())) + '.rst',
                  'w+') as f:
            f.write(str(nestml_model_doc))
