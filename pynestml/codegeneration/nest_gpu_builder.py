...
class NESTGPUBuilder(Builder):
    ...
    def build(self) -> None:
        ...
        try:
            subprocess.check_call(autoreconf_cmd, stderr=subprocess.STDOUT, cwd=str(os.path.join(target_path)))
        ...
        try:
            subprocess.check_call(config_cmd, stderr=subprocess.STDOUT, cwd=str(os.path.join(target_path)))
        except subprocess.CalledProcessError as e:
            raise GeneratedCodeBuildException(
        ...
        # now execute make
        try:
            subprocess.check_call(make_cmd, stderr=subprocess.STDOUT, cwd=str(os.path.join(target_path)))
        except subprocess.CalledProcessError as e:
            raise GeneratedCodeBuildException(
        ...
        try:
            subprocess.check_call(make_install_cmd, stderr=subprocess.STDOUT, cwd=str(os.path.join(target_path)))
        except subprocess.CalledProcessError as e:
            raise GeneratedCodeBuildException(
